...
public final class ModItems {
    ...
    public static void registerCompostables() {
        CompostableRegistry compostables = CompostableRegistry.INSTANCE;
        compostables.add(RED_POPLAR_LEAVES.get(), 0.3F);
        compostables.add(ORANGE_POPLAR_LEAVES.get(), 0.3F);
        compostables.add(YELLOW_POPLAR_LEAVES.get(), 0.3F);
        compostables.add(POPLAR_SAPLING.get(), 0.3F);
        compostables.add(RED_SHRUB.get(), 0.3F);
        compostables.add(SHELF_MUSHROOM.get(), 0.65F);
    }

    public static void registerFuels() {
        FuelValueEvents.BUILD.register((builder, context) -> {
            int baseSmeltTime = context.baseSmeltTime();
            int quarterSmelt = baseSmeltTime / 4;
            int halfSmelt = baseSmeltTime / 2;
            int oneAndHalfSmelts = baseSmeltTime * 3 / 2;

            builder.add(POPLAR_SAPLING.get(), halfSmelt);
            builder.add(POPLAR_FENCE.get(), oneAndHalfSmelts);
            builder.add(POPLAR_FENCE_GATE.get(), oneAndHalfSmelts);
            builder.add(POPLAR_TRAPDOOR.get(), oneAndHalfSmelts);
            builder.add(POPLAR_BUTTON.get(), halfSmelt);
            builder.add(POPLAR_PRESSURE_PLATE.get(), oneAndHalfSmelts);

            for (DyeColor color : DyeColor.values()) {
                builder.add(WOOL_STAIRS.get(color).get(), halfSmelt);
                builder.add(WOOL_SLABS.get(color).get(), quarterSmelt);
            }
        });
    }
