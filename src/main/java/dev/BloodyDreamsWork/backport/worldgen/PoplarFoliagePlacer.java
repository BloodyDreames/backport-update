...
public class PoplarFoliagePlacer extends FoliagePlacer {
    public static final MapCodec<PoplarFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(
            instance -> foliagePlacerParts(instance)
                    .and(instance.group(
                            IntProviders.codec(5, 16).fieldOf("height")
    ...
    @Override
    protected void createFoliage(WorldGenLevel level, FoliageSetter blockSetter, RandomSource random,
    ...
    private void placeLeavesRow(WorldGenLevel level, FoliageSetter blockSetter, RandomSource random,
    ...
    private void replaceLeavesWithLog(WorldGenLevel level, FoliageSetter blockSetter,
    ...
    private static void tryPlaceLog(WorldGenLevel level, FoliageSetter blockSetter,
        ...
        if (level.isStateAtPosition(pos,
                state -> state.equals(config.foliageProvider.getState(level, random, pos)))) {
            blockSetter.set(pos, stateModifier.apply(config.trunkProvider.getState(level, random, pos)));
        }
    }
