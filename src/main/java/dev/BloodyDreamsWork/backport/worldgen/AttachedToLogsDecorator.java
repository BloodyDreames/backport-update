...
public class AttachedToLogsDecorator extends TreeDecorator {
    ...
    public static final MapCodec<AttachedToLogsDecorator> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    protected TreeDecoratorType<?> type() {
        return ModWorldgenTypes.ATTACHED_TO_LOGS_DECORATOR.get();
    }
    ...
    public void place(Context context) {
        ...
        for (BlockPos logPos : context.logs()) {
            ...
            Direction direction = this.directions.get(random.nextInt(this.directions.size()));
            BlockPos target = logPos.relative(direction);
            if (context.isAir(target)) {
                context.setBlock(target, this.blockProvider.getState(context.level(), random, target));
            }
        }
    }
}
