...
public class FallenTreeFeature extends Feature<FallenTreeConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<FallenTreeConfiguration> context) {
        ...
        placeLog(level, origin, config.trunkProvider().getState(random, origin), Direction.Axis.Y);
        for (BlockPos pos : logs) {
            placeLog(level, pos, config.trunkProvider().getState(random, pos), direction.getAxis());
        }
